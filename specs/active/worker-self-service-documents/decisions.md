...
## Revision note (resend/recover access link — operational gap found during Phase 11 local testing)

Local end-to-end testing of Phase 11 (`tasks.md`) surfaced a real operational gap not covered by the original design: a worker who receives an invite email, opens the link, but closes the tab before setting a password, has no way to get a working link again — the original invite token is single-use and now spent, `create-worker-account` returns `already_linked` on a retry (correctly, since the `auth.users`/`profiles` rows already exist), and the worker is stuck linked-but-passwordless. The same gap applies to a worker who successfully activated their account earlier and later simply forgets their password. Neither case was an open question before this revision; both are new. This adds decisions #30–34 below: a new, separate admin action ("Reenviar enlace de acceso") and a new, separate Edge Function (`resend-worker-access-link`) — **not** a change to `create-worker-account`, **not** a removal of "Vincular cuenta existente", and **not** the general self-service "forgot password" flow that remains out of scope. Decision #34 also resolves, rather than leaves open, the one design question this addition raised (whether a single `current_app_role()` check without a backing RPC is a sufficient security boundary — it is). No code is written yet; this is spec-only, same as the original Phase 11 revision. See the updated "Scope" and new acceptance-criteria subsection in `spec.md`, the new `implementation-plan.md` §12, and the new Phase 11C in `tasks.md`.
...
## 30. "Reenviar enlace de acceso": a separate Edge Function, not a `create-worker-account` branch, and not the general self-service forgot-password flow

Local testing of Phase 11 found a real gap (see the revision note above): a worker whose invite link goes stale before they set a password — or who later simply forgets their password — has no way to get a working `/set-password` link again. `create-worker-account` cannot be reused for this as-is, because its whole design (decision #21, case 6) is built around *not yet linked* — the moment a `profiles` row exists for a worker, it short-circuits to `"already_linked"` and does nothing further, by design, to avoid re-inviting or re-touching an already-provisioned account. Bolting a "resend anyway" branch onto that function would mean re-deriving, inside the same request handler, a second precondition (*must already be linked*) that is the exact opposite of the first function's guard (*must not already be linked*) — two mutually-exclusive preconditions in one function is a sign it should be two functions, not one with more branches.

So this adds a new, separate Edge Function, `resend-worker-access-link`, with its own admin action button, **"Reenviar enlace de acceso"**, alongside — never replacing — "Crear cuenta de acceso" and "Vincular cuenta existente". All three coexist permanently:

- **"Crear cuenta de acceso"** (`create-worker-account`): for a worker with **no** linked account yet. Invites or links, then calls `link_worker_account`.
- **"Reenviar enlace de acceso"** (`resend-worker-access-link`, new): for a worker who **already has** a linked account (`profiles.role = 'worker'`) but needs a fresh password-setup/recovery link — either because their original invite link went stale, or because they forgot their password later. Never creates or touches a `profiles` row.
- **"Vincular cuenta existente"** (`link_worker_account`, manual): the admin-typed-email fallback, unchanged (decision #4).

This is also **not** the general self-service "forgot password" flow that `spec.md`'s Out-of-scope section excludes. That excluded flow is worker-initiated (a public "forgot your password?" link on the login page, available to any active account, no admin involved). `resend-worker-access-link` is the opposite shape on every axis that matters: **admin-initiated**, **admin-authenticated and admin-checked server-side**, **scoped to one specific `workerId`** the admin already has selected in the UI, and it exists to close an operational gap in the invitation flow this feature itself introduces — not to build general account-recovery infrastructure. `spec.md`'s Out-of-scope bullet for general self-service recovery is narrowed to make this distinction explicit, mirroring how decision #27 already carved `/set-password` itself out of that same exclusion for the same kind of reason.

## 31. Reused redirect target: `WORKER_INVITE_REDIRECT_URL` covers both invite and resend/recovery links, not a second env var

Both the original invite email and the new resend/recovery email land the worker on the exact same page, for the exact same purpose: `/set-password`, to finish setting or update a password before continuing. Rather than introduce a second, parallel secret (`WORKER_PASSWORD_REDIRECT_URL`) that would have to be kept equal to the first in every environment forever, `resend-worker-access-link` reads and reuses the same `WORKER_INVITE_REDIRECT_URL` already established by decision #25 and already configured (`.env.local`, `.env.example`, and — once deployed — the remote `supabase secrets set`) for `create-worker-account`.

Rejected alternative: renaming to `WORKER_PASSWORD_REDIRECT_URL` (floated as an option precisely because "invite" reads slightly oddly for a password-*recovery* email). Rejected because it would require touching the already-implemented, already-verified `create-worker-account` function and its entire existing environment wiring (`.env.local`, `.env.example`, `supabase/config.toml`, decisions.md #25–26, `tasks.md`'s Phase 11A/11B verification notes) purely for a cosmetic rename with zero behavior change — and it would introduce exactly the "two variables that must always be set to the same value, forever, in every environment" risk this decision avoids by reusing one. If a real future need arises for the two flows to land on genuinely different URLs, split the variable then, with a concrete reason — not preemptively now.

## 32. Minimal-privilege client choice: `resend-worker-access-link` never reads or uses the service-role key

Unlike `create-worker-account` (decision #21), which needs the service-role client for exactly one thing — `inviteUserByEmail`, an Admin-API-only capability — `resetPasswordForEmail` is a normal, publicly-available GoTrue endpoint that only ever requires the anon key; it is not part of the Admin API and does not need `SUPABASE_SERVICE_ROLE_KEY` at all. `resend-worker-access-link` therefore constructs only:

1. A **user-scoped** client (the caller's forwarded `Authorization` header) — used for the admin check (`current_app_role()`), the `workers.email` read, and the `profiles` linked-status read. Exactly the same pattern as `create-worker-account`'s user-scoped client.
2. A **plain anon-key client**, with no forwarded caller header — used **only** for the `resetPasswordForEmail` call itself, deliberately kept separate from the admin's own authenticated client so the admin's session token is never sent alongside an unrelated, unauthenticated-by-design public Auth action.

`Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")` is never read by this function. This is a strictly smaller privilege footprint than `create-worker-account`, worth calling out explicitly rather than reflexively copying the two-client (user + service-role) shape from decision #21 where it isn't actually needed.

## 33. Preconditions and no-enumeration: this function never links, unlinks, or creates — and never accepts a caller-supplied email

Concrete precondition, checked before any email is sent:

- If no `profiles` row exists for this `worker_id` at all, reject with a clear "este trabajador no tiene una cuenta vinculada todavía; usa 'Crear cuenta de acceso' primero" error, directing the admin to the correct action — no email is sent, no `profiles` row is touched.
- If a `profiles` row exists for this `worker_id` but its `role` is not `worker` (should not normally occur, given `profiles.worker_id` is only ever set alongside `role = 'worker'`, but checked defensively anyway), reject the same way — this action is specifically about worker accounts.
- This function **never** calls `link_worker_account` or `unlink_worker_account`, and never writes to `profiles` in any way, under any code path. It only ever triggers `resetPasswordForEmail`. This is the direct implementation of the request's explicit rule: "Do not create profiles in this resend/recovery function. Do not unlink/relink the worker."

No email-enumeration risk, and by construction rather than by adding probing-prevention logic of its own: the request body is `{ workerId }` only — never an `email` field, extending decision #29's reasoning to this function too — so there is no way for a caller to ask "does this arbitrary email exist?" in the first place; the email is always resolved from `public.workers.email` for a specific worker the admin already selected, gated by the admin-only check above. On top of that structural property, GoTrue's `resetPasswordForEmail` itself already returns a uniform response regardless of whether the target address has a matching `auth.users` row, which this function relies on rather than re-implementing.

## 34. The Edge Function's own `current_app_role()` check is the real security boundary here, not merely a UX fast-fail

`create-worker-account` (decisions.md #21, #24 case 7) has two layers: an early `current_app_role()` call inside the Edge Function (fast, clear UX — avoids a wasted invite email) *and* `link_worker_account`'s own internal admin check (the real, redundant-by-design enforcement, since that RPC runs regardless of whether the Edge Function's own check was somehow buggy or skipped). `resend-worker-access-link` has no equivalent RPC to call — it never calls `link_worker_account` (decision #33) — so there is only one check, not two.

This is not a weaker guarantee. `current_app_role()` is a `SECURITY DEFINER` SQL function that resolves the caller's role from their actual `profiles` row, keyed off `auth.uid()` from their own cryptographically-verified, forwarded JWT — it is not a client-supplied claim the Edge Function has to trust blindly, and it is the exact same function every other authorization check in this entire feature already relies on as ground truth (RLS policies, `RoleGate.jsx` via `useProfile()`, and — one layer down — `link_worker_account`'s own internal check, which itself is nothing more than a call to this same function). A single call to it, made via the user-scoped client with the caller's real forwarded `Authorization` header, is exactly as trustworthy here as it is everywhere else it's used; the two-layer pattern in `create-worker-account` is defense-in-depth on top of an already-trustworthy check, not evidence that a single call to `current_app_role()` is insufficient on its own. `resend-worker-access-link` therefore has a real, non-bypassable admin boundary with a single check — this is a deliberate, resolved design choice, not an open question left for implementation time.
