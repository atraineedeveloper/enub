...
- [ ] Disable "Crear cuenta de acceso" in the UI (with a hint) when `worker.email` is empty — **not implemented**: the shared `Menus.Button` component (`src/ui/Menus.jsx`) does not accept/forward a `disabled` prop, and adding one was out of scope for this pass (a widely-used shared primitive, touching it risked side effects elsewhere). Relying instead on the Edge Function's existing clear error message for this case, which is already verified live (empty-email worker → "Este trabajador no tiene correo registrado..." toast). Convenience-only gap, not a security gap — the function still blocks it either way.
...
- [x] Add admin-only "Crear cuenta de acceso" action to `WorkerRow.jsx`; relabel the existing "Vincular cuenta" to "Vincular cuenta existente" to read as the fallback — kept permanently (decisions.md #4), not removed once the automatic flow ships.
- [ ] Disable "Crear cuenta de acceso" in the UI (with a hint) when `worker.email` is empty — **not implemented**, see note under Phase 11A above (`Menus.Button` has no `disabled` prop; the Edge Function's error message is the relied-upon safeguard, already verified live for this exact case).
- [x] Add `createWorkerAccount({ workerId })` to `src/services/apiProfiles.js`.
- [x] Create `src/features/authentication/useCreateWorkerAccount.js`.
...
- [x] Confirmed no `service_role` reference appears anywhere in `src/` (unchanged from Phase 11A/11B's standing check — `src/` was not touched this pass at all).
- [x] Confirmed no remote deploy, `supabase link`, or `supabase secrets set` was run while implementing/testing this locally.

**Not done in 11C-1 (see Phase 11C-2 below):**
...
- [ ] Local verification: full click-through from the UI (not just direct `curl`/Edge-Function calls) — worker with a linked profile but no password set → clicking "Reenviar enlace de acceso" sends a new email, confirmed in Mailpit, and the worker can complete `/set-password` and log in with the new password.
- [ ] Confirm "Crear cuenta de acceso" and "Vincular cuenta existente" UI behavior is unchanged after this addition (regression check).
