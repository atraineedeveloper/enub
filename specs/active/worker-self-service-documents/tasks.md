...
**Sub-phase 11B-1: the `/set-password` companion page — done.** The admin-facing "Crear cuenta de acceso" button (11B-2) is still pending — do not treat Phase 11B as a whole as complete.
...
- [ ] Local verification: confirm the invite email's actual content in Mailpit (recipient, valid link, redirect to local `WORKER_INVITE_REDIRECT_URL`, never production) — not just that the API call succeeded (decisions.md #28, resolved). **Now fully covered**: recipient/link/redirect-target were all confirmed directly from the real Mailpit message and the GoTrue redirect trace during 11B-1's testing above. Left unchecked here only because the trigger was a direct Edge Function `curl` call, not yet the actual admin-facing UI button (11B-2) — re-confirm once that button exists, as a full browser-driven click-through rather than an API call standing in for it.
...
**Sub-phase 11B-2: the admin-facing button — not started.**
