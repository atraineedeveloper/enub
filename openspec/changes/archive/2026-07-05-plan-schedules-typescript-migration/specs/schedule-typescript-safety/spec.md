# Capability: Schedule TypeScript Safety

Behavior-preservation contract for converting `src/features/schedules/**`
from JavaScript/JSX to TypeScript/TSX. This capability defines no new
user-facing functionality — it constrains how the migration must be carried
out so existing schedule behavior, correct or already-buggy, is not silently
changed as a side effect of adding types.
