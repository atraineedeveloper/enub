...
- Task 5.5 (manual smoke test) and 5.6 (recording its results): **not
  performed** — 23 of this file's 25 tasks are complete; these are the 2
  remaining. No browser/dev-server session was available in any
  implementation pass in this change. The manual visual testing that *did*
  happen (which found and reported the missing-error-message bug and the
  missing-lower-bound bug, both now fixed and documented above) was
  targeted spot-checking of specific reported symptoms, not the full 5.5
  checklist — it does not substitute for 5.5/5.6. Still outstanding before
  this change is complete: grade labels verified across ≥2 selected
  semesters for groups with different `year_of_admission` values; all 4 PDF
  exports checked; `/groups` confirmed unaffected; the browser console
  checked for unexpected `console.warn` fallback output against real
  `semester` records in both formats; and the lower-bound exclusion
  (task 2.2's fix) visually confirmed in the actual dropdown, not just via
  the ad hoc formula script.
