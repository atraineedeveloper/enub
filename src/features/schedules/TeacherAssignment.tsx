...
function TeacherAssignment({ workers, scheduleTeachers, scheduleAssignments }: TeacherAssignmentProps) {
  ...
  const countTeacherSchedules = useMemo(() => {
    return filteredSchedulesTeacher.reduce((acc: any, item: any) => {
  ...
  const totalHours = useMemo(() => {
    ...
    uniqueTeacherSchedule.forEach(
      (schedule: any) => (total += schedule.quantity * 2)
    );
    return total;
  ...
  return (
    <>
      <Select id="worker_id" onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleWorkerChange(e.target.value)}>
      ...
      <Table role="table">
        ...
        {uniqueTeacherSchedule.map((schedule: any) => (
